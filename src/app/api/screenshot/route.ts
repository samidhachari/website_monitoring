...
async function takeScreenshot(url: string, id: number): Promise<string | null> {
  ...
  try {
    ...
    // Capture buffer instead of writing immediately
    ...
    if (serviceKey && storageBucket && supabaseUrl) {
      try {
        ...
        if (uploadError) {
          ...
          // Note: using a constructed public URL because getPublicUrl requires anon key; direct construction works for public buckets
          return publicUrl;
        }
    ...
    // Fallback: write to local downloads and public folder
    try {
